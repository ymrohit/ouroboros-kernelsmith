@triton.jit
def _k(x_ptr, w_ptr, y_ptr, stride, N, eps, alpha, BLOCK: tl.constexpr):
    ...
    for off in range(0, N, BLOCK):
        ...
        xhat = x * rms * w
        # ELU: alpha * (exp(xhat) - 1) if xhat < 0 else xhat
        y = tl.where(xhat >= 0, xhat, alpha * (tl.exp(xhat) - 1.0))
        tl.store(y_ptr + cols, y, mask=MM)
def run(x, w):
    M, N = x.shape; y = torch.empty_like(x)
    _k[(M,)](x, w, y, x.stride(0), N, 1e-6, 1.0, BLOCK=4096)
    return y