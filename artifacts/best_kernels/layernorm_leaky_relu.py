@triton.jit
def _k(x_ptr, w_ptr, b_ptr, y_ptr, stride, N, eps, negative_slope, BLOCK: tl.constexpr):
    ...
    for off in range(0, N, BLOCK):
        ...
        xhat = x * rr * w + b
        y = tl.where(xhat > 0, xhat, xhat * negative_slope)
        tl.store(y_ptr + cols, y, mask=MM)
def run(x, w, b):
    M, N = x.shape; y = torch.empty_like(x)
    _k[(M,)](x, w, b, y, x.stride(0), N, 1e-5, 0.01, BLOCK=4096)
    return y