...
def _k(x_ptr, r_ptr, w_ptr, b_ptr, y_ptr, stride, N, eps, BLOCK: tl.constexpr):
    ...
    cols2 = tl.arange(0, BLOCK)
    w = tl.load(w_ptr + cols2, mask=cols2 < N, other=0.0).to(tl.float32)
    b = tl.load(b_ptr + cols2, mask=cols2 < N, other=0.0).to(tl.float32)
    x_tilde = x_norm * w + b
    # tanh epilogue
    tanh = 2.0 * tl.sigmoid(x_tilde * 2.0) - 1.0
    tl.store(y_ptr + cols2, tanh, mask=cols2 < N)
def run(x, residual, w, b):
    M, N = x.shape; y = torch.empty_like(x)
    _k[(M,)](x, residual, w, b, y, x.stride(0), N, 1e-6, BLOCK=triton.next_power_of_2(N))
    return y