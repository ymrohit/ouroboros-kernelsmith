# GOLD seed kernel: fused rotary embedding (LLaMA rotate_half).
# out[:h] = x1*cos1 - x2*sin1 ;  out[h:] = x2*cos2 + x1*sin2   (h = D/2)
# One pass per row, fp32 math. Eager does cat + 2 muls + add as separate launches.
@triton.jit
def _rope_kernel(x_ptr, cos_ptr, sin_ptr, y_ptr, stride, D, BLOCK: tl.constexpr):
    row = tl.program_id(0)
...
def run(x, cos, sin):
    ...
    _rope_kernel[(M,)](x, cos, sin, y, x.stride(0), D, BLOCK=triton.next_power_of_2(D // 2))
    return y
