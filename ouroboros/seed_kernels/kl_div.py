# GOLD seed kernel: per-row KL(softmax(x) || softmax(y)) from raw logit pairs.
# Both logsumexps max-subtracted; out = sum(exp(lx) * (lx - ly)). fp32 throughout.
@triton.jit
def _kl_kernel(x_ptr, y_ptr, o_ptr, sx, sy, N, BLOCK: tl.constexpr):
    ...
    my = tl.full([BLOCK], -float("inf"), dtype=tl.float32)
    for off in range(0, N, BLOCK):
        cols = off + tl.arange(0, BLOCK)
        x = tl.load(x_ptr + cols, mask=cols < N, other=-float("inf")).to(tl.float32)
        y = tl.load(y_ptr + cols, mask=cols < N, other=-float("inf")).to(tl.float32)
        mx = tl.maximum(mx, x)
        my = tl.maximum(my, y)
    rx = tl.max(mx)
    ry = tl.max(my)
    ...
    for off in range(0, N, BLOCK):
        ...
        dy += tl.where(mask, tl.exp(y - ry), 0.0)
    lse_x = rx + tl.log(tl.sum(dx))
    lse_y = ry + tl.log(tl.sum(dy))
