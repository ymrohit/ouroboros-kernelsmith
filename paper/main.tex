...
A three-seed ablation on a 1B model finds the training-recipe arms statistically
indistinguishable on familiar operators, with best-of-$N$ search against the referee
...
are statistically indistinguishable (three seeds, on a 1B model), with search against the
referee accounting for most of the result. Learning matters on operators the model has not
...
and ran on a single RTX~4090. The arms are statistically indistinguishable
(Table~\ref{tab:ablmc}, Fig.~\ref{fig:ablmc}): the spread of geometric means across arms,
$\mcSpread$, is below twice the largest per-arm standard deviation across seeds,
$\mcMaxSeedStd$. The cross-check settles it. Distill-only was nominally the best arm in the
...
\bottomrule
\end{tabular}
\caption{Multi-seed ablation on MiniCPM5-1B, three seeds per arm. The arms are
indistinguishable (spread $\mcSpread \le 2\times$ the per-arm seed standard deviation
$\mcMaxSeedStd$), and every arm beats max-autotune. The 1B model writing compiler-beating
...
three-seed. (4) KernelBench cross-evaluation is
...
27B parameters with no change to the pipeline, and the results did not depend on which model
filled that slot. The referee did. Where verification is cheap and the quality signal is
