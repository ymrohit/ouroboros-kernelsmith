...
exploiting the evaluation harness rather than accelerating the hardware. We take the root
cause to be architectural. The reward channel is usually treated as a metric, when it has to
be built and tested as something an adversary will try to fool. \sys{} is a
...
fusion kernels that all beat \maxauto{} reproducibly: \stabDiscoveries{} of \stabOps{} pass
a five-run stability gate (the weakest at $\stabWeakestMean\times\pm\stabWeakestSpread$, the
strongest at $\stabStrongestMean\times\pm\stabStrongestSpread$), and each holds a
geometric-mean win across a shape grid of \gridCellsTotal{} cells, with per-cell recompiled
baselines, a cache-cold check, and the \gridLossPct\% of cells we lose reported
individually. A three-seed ablation on a 1B model finds the four training-recipe arms
statistically indistinguishable on familiar operators, with best-of-$N$ search against the
referee accounting for nearly all of the loop's value; learning matters instead on operators
the model has not seen, where validity rises from roughly 50\% to 68\% within one run. We
also record \falsifications{} cases in which the referee contradicted a belief we held in
advance, among them our own diagnosis of the system's one weak regime, which the model
repaired with a simpler schedule than we had predicted, turning the worst measured loss
($\beforeALS\times$) into a $\invALSshortMA\times$ win. We release the harness, the kernels,
the corpus, the adapters, the raw measurements, and the record of those predictions.
\end{abstract}
...
that cannot be gamed, an open model out-searches the compiler's own autotuner and repairs
its own weak cases. Every number in the paper is regenerated from a harness output by a
script that also checks the reports against each other; a number that cannot be regenerated
this way does not appear.
...
every kernel model-authored, explore-arm zero, validity $0.713$). The product's single
characterized weakness, the short-row regime that accounts for the large majority of the
shape grid's loss cells, is closed for the chain family on this evidence. This is the
generalization claim in its strongest form: the discovery is a capability the model carries
to unseen operators, not a memorized output.
