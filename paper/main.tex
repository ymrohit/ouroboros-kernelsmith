...
\date{June 2026 \quad (preprint; ablations replicated at 3 seeds on a 1B model)}
...
an \emph{adversarial system under test}. We present \sys{}, a verifier-grounded
...
lose reported individually. Ablations (replicated at three seeds on a 1B model, OpenBMB MiniCPM5-1B, on a free
RTX~4090) show that on familiar operators the four training-recipe arms are
\emph{statistically tied}: frozen best-of-$N$ search captures essentially all of the full
loop's value, and no learning ingredient separates beyond seed noise. On never-seen
operators learning is visibly load-bearing instead: validity climbs from $\sim$50\% to 68\%
within one run as the model distills its own verified solutions. Finally, we document nine instances in which the
verifier falsified a confident belief held by the system's own builders. One was our
...
operators the recipe's arms are statistically tied (three seeds, on a 1B model) ---
search against the referee dominates and no learning ingredient separates. On foreign
operators learning is load-bearing and visible within a single run
...
builders. Every one of those statements is regenerable from a machine-emitted
...
ran unchanged across a 2B model (an earlier campaign) and the 27B reported here; we do not claim a controlled scale comparison, only architectural model-agnosticism. We also intentionally evaluate against the
...
per arm invites the objection ``$n{=}1$.'' We replicated the ablation at \textbf{three
independent seeds} per arm, on a different model and a different scale: OpenBMB's
\textbf{MiniCPM5-1B} (a 1B open model), SFT'd to the validity gate in \mcSftEpochs{} epochs
(\mcSftValid\% valid) and run entirely on a single consumer RTX~4090 at zero marginal cost.
The arms are \emph{statistically tied} (Table~\ref{tab:ablmc}, Fig.~\ref{fig:ablmc}):
between-arm spread $\mcSpread$ is below twice the largest within-arm seed standard deviation
$\mcMaxSeedStd$. The decisive cross-check: \textbf{distill-only was nominally \emph{best} in
the 27B single-seed run and is nominally \emph{worst} ($\mcDistillonlyGm$) in the 1B
three-seed average} --- the same arm at opposite ends, which is the cleanest possible proof
that the single-seed ordering was noise. \texttt{no-learn} (pure best-of-$N$ from the frozen
SFT model, \emph{no learning at all}) sits at $\mcNolearnGm$, within noise of control's
$\mcControlGm$.
...
\bottomrule
\end{tabular}
\caption{Multi-seed ablation, \textbf{MiniCPM5-1B}, 3 seeds, free RTX~4090. Arms tied
(spread $\mcSpread \le 2\times$ seed-std $\mcMaxSeedStd$); every arm beats max-autotune.
A 1B model writing compiler-beating kernels is itself the result of the sponsor-scale
run (\S\ref{sec:bounds}).}\label{tab:ablmc}
\end{table}
...
The conclusion that survives error bars is the robust one, not the fragile ordering:
\textbf{(1)} On familiar operators, the referee plus best-of-$N$ sampling captures
essentially all of the full loop's geomean --- \emph{search, not learning, is the dominant
force where competence exists}, and this now holds across two model scales (1B and 27B).
\textbf{(2)} No single learning ingredient --- feedback, the policy-gradient term, or
learning at all --- separates beyond seed noise here. \textbf{(3)} Learning's value
concentrates on \emph{foreign} operators: the \invValidRate{} vs $\sim$0.69 validity gap and
Fig.~\ref{fig:learning} are the controlled contrast, and a 4.5-hour \emph{continue-SFT} on
new operators stalled and degraded proven ones while RL-from-verifier taught them cleanly.
Corpus imitation is not how the new capabilities arrived.
...
our own stability gate under a written provenance note). The ledger cuts both ways: one pre-registered
prediction (that the loss-regime repair would \emph{transfer}) was \emph{ratified}, $10/10$
(\S\ref{sec:invention}). We highlight this practice not as housekeeping but as the paper's
second thesis: \textbf{a system with an honest referee does not require its builders to be
right; the referee, not their confidence, decides.} The ledger, with open pre-registered
predictions, ships in the artifact.
...
(2b) \emph{Scale, downward:} the whole pipeline was re-run on OpenBMB's
\textbf{MiniCPM5-1B} (a 1B open model) on a single consumer RTX~4090 at zero marginal cost;
it reached the validity gate in \mcSftEpochs{} epochs (\mcSftValid\% valid) and its kernels
beat \maxauto{} on every benchmarked operator (geomean $\sim\mcControlGm\times$,
Table~\ref{tab:ablmc}). A \emph{1-billion-parameter} model writing verified compiler-beating
kernels on commodity hardware is the sharpest statement of the thesis: the model is
replaceable down to 1B; the referee is not.
(3) The 27B ablations are single-seed (the 1B replication of Table~\ref{tab:ablmc} is
