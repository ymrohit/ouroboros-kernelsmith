...
this referee, an open 27B model (Qwen3.6-27B with LoRA) produced \productKernels{} Triton
fusion kernels; the \stabOps{} we carry through a five-run stability gate \emph{all} beat
\maxauto{} reproducibly, and each also holds a geometric-mean win across a shape grid whose
\gridLossPct\% losing cells we report individually (the remaining \numKernelsExplore{}
kernels come from an exploratory run held to a different baseline, \S\ref{sec:results}).
